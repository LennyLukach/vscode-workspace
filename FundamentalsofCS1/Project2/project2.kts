...
fun testReadTaggedFlashCardsFile() {
    ...
    testSame(readTaggedFlashCardsFile("example_tagged.txt"), exampleList, "example_tagged.txt")
}
...
fun <T : IMenuOption> chooseMenuOption(options: List<T>): T? {
    ...
    fun keepIfValid(input: String, validIndex: IntRange):Int {
    ...
    fun transitionOptionChoice(state: Int, kbInput: String): Int {
...
fun <T> topK(items: List<T>, k: Int, eval: EvaluationFunction<T>): List<T> {
...
fun levenshteinDistance(s1: String, s2: String): Int {
    ...
    fun levenshteinDistanceHelper(s1: String, s2: String, s1Index: Int, s2Index: Int): Int {
        return when {
...
fun testStudyStateToText() {
    ...
    testSame(studyStateToText(testState2), "Los Angeles\nThink of the result? Press enter to continue", "testState2")
}

// returns the next state given the current state and keyboard input
fun nextStudyState(state: StudyState, kbInput: String, classifier: PositivityClassifier): StudyState {
    val deck = state.deck
...
fun testIsDone() {
    ...
    testSame(isDone(testState3), false, "testState3")
    testSame(isDone(testState4), false, "testState4")
}
...
fun study2(){
    val options = listOf(
        NamedMenuOption(TFCListDeck(readTaggedFlashCardsFile("example_tagged.txt"), 0,  DeckState.QUESTION), "Read from File"),
    ...
    when(deck){
        null -> null
            ...
            val classifier = chooseMenuOption(options2)
            when(classifier){
                null -> null
                else -> {
